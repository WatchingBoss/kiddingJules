...
class BookManagerWindow(QMainWindow):
    ...
    def add_row(self, book):
        ...
        item_title.setData(Qt.UserRole, book) # Store book object in first item
        self.table.setItem(row, 0, item_title)

        # Year (Sortable as number)
        item_year = QTableWidgetItem()
        item_year.setData(Qt.DisplayRole, book.year)
    ...
    def apply_filters(self):
        ...
        for row in range(self.table.rowCount()):
            book = self.table.item(row, 0).data(Qt.UserRole)
